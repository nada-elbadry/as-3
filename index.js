...
submitBtn.addEventListener("click",function(){
 ...
 else
 {
    Swal.fire({
        title: "Invalid Data",
        text: `${siteNameInput.value == "" ?"please enter site name":""} ${validateUrl() == false ?"Please enter  valid url":""}`,
        icon: "error"
      });
  }
})
    ...
    for(var i = 0 ; i < arr.length ; i++){
       cartoona+=`
                <tr>
                    ...
                    <td><button class="btn bg-update" onclick="preUpdate(${i})"> <i class="fa-regular fa-pen-to-square pe-2"></i> Update</button></td>
                    <td><button class="btn bg-delete" onclick="deleteBookMark(${i})"><i class="fa-solid fa-trash-can pe-2"></i>Delete</button></td>
                </tr>
       `;
    }
